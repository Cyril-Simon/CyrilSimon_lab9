...
void insertRecord(struct HashType *hashTable, struct RecordType *record, int tableSize)
{
  ...
  int index = hash((int)record->name, tableSize);
    ...
    if(record->name[index].id == NULL)
    {
      record->name = hashTable[index];
    }
...
void displayRecordsInHash(struct HashType *hashTable, int tableSize)
{
    ...
    {
      printf("Index %d -> %d, %s, %d -> %d", i, hashTable->rec->id, hashTable->rec->name, hashTable->rec->order, hashTable->rec->id);
    }
...
int main(void)
{
    ...
    struct HashType *htable;

    htable = calloc(sizeof(struct HashType), hashTableSize);
    // for each record in pRecords, insert it into the hash table using the insertRecord function
    for(int i = 0; i < pRecords; i++)
    {
      insertRecord(htable, pRecords, hashTableSize);
    }
    // call the display records function
    displayRecordsInHash(htable, hashTableSize);
