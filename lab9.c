...
void insertRecord(struct HashType *hashTable, struct RecordType *record, int tableSize)
{
  ...
  int index = hash((int)record->id, tableSize);
    ...
    struct RecordType *current = hashTable[index].rec;

    if(current == NULL)
    {
      hashTable[index].rec = record;
    }
...
void displayRecordsInHash(struct HashType *hashTable, int tableSize)
{
    ...
    for(int i = 0; i < tableSize; i++)
    {
      ...
      struct RecordType *current = hashTable[i].rec;
...
int main(void)
{
    ...
    {
      insertRecord(htable, &pRecords[i], hashTableSize);
    }
